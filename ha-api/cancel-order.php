...
if (!in_array((string) $order['status'], ['placed', 'paid', 'preparing', 'ready'], true)) {
    echo json_encode(['success' => false, 'error' => 'Cannot cancel this order']);
    exit;
}

fm_order_set_status($pdo, $orderId, 'cancelled', $hotelId);
try {
...
echo json_encode(['success' => true, 'order_id' => $orderId, 'status' => 'cancelled']);
